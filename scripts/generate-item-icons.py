...
ICON_SPECS = (
    ...
    IconSpec(
        ...
        asset_type="website backpack game UI consumable item icon",
        primary_request="为网页背包系统绘制一枚“调序检查券”游戏道具图标，作用是在免费月度调序检查配额用尽后额外执行一次调序检查。第一视觉含义是“重新排列 + 检查验证”。",
    ...
    IconSpec(
        ...
        asset_type="website backpack game UI consumable item icon",
        primary_request="为网页背包系统绘制一枚“追加重算券”游戏道具图标，作用是在有效修订窗口内额外执行一次重算。第一视觉含义是“再计算一次 + 增加一次机会”。",
