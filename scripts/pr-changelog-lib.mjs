  ...
  return [
    {
      ...
      content: [
        ...
        '分类时必须先判断受众，再判断 feature、fix、performance 或 security；“普通用户”不包括管理员、运维人员和开发者。仅在管理后台、管理员登录后页面或内部工具中可见的变化，即使属于浏览器页面、交互体验或性能优化，也不得放入 public_sections。',
        'internal_sections 记录不应在网站展示但需要在仓库追溯的变化：管理后台页面的功能、修复和优化一律用 admin，部署/运维/CI/构建用 operations，重构/测试/文档/依赖/开发维护用 maintenance。',
        '硬性示例：管理后台支持公告草稿自动保存，以及自动保存频率、防抖或性能优化，只能放入 internal_sections 的 admin；不得归入 public_sections 的 feature、fix 或 performance。无法确认普通用户是否能直接感知时，默认归入 internal_sections，不要猜测为公开变更。',
