...
describe('PricingPage', () => {
  it('renders the two public SKUs and full disclosure policy', () => {
    ...
    expect(screen.getByText(/滚动 7 天窗口，最多成功更新 2 次/)).toBeInTheDocument()
    expect(screen.getByText(/设备 Token、浏览器 User-Agent 和网络 IP 前缀/)).toBeInTheDocument()
    expect(screen.getByText(/2 个工作日内首次响应/)).toBeInTheDocument()
    const contactLinks = screen.getAllByRole('link', { name: '联系客服' })
    expect(contactLinks.some((link) => ['inline-flex', 'items-center', 'justify-center'].every((className) => link.classList.contains(className)))).toBe(true)
