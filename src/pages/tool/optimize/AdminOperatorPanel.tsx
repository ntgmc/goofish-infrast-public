    ...
    <details className="tool-panel overflow-hidden">
      <summary className="tool-panel-header flex min-h-12 cursor-pointer list-none items-center justify-between gap-4 px-5 py-4 text-sm font-semibold text-ink-primary transition-colors duration-150 hover:bg-surface-2/60 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/45 sm:px-6">
        <span className="flex flex-wrap items-center gap-2">
          {copy.optimize.pages_tool_optimize_AdminOperatorPanel_001}<span className="tool-status tool-status--current">
            {operatorCount} {copy.optimize.pages_tool_optimize_AdminOperatorPanel_002}</span>
        </span>
        <span className="tool-status">{copy.optimize.pages_tool_optimize_AdminOperatorPanel_003}</span>
      </summary>
      <div className="border-t border-surface-3/60 p-5 sm:p-6">
        <p className="text-sm leading-6 text-ink-secondary">
          {copy.optimize.pages_tool_optimize_AdminOperatorPanel_004}</p>
        <input
          ref={fileRef}
          type="file"
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          ...
          >
            {copy.optimize.pages_tool_optimize_AdminOperatorPanel_005}</button>
