...
describe('metered billing quote', () => {
  ...
  it('reports sufficient balance when the free balance covers the personal charge', async () => {
    mocks.issueMeteredScheduleQuote.mockResolvedValue(quote({ available: '1200.00', sufficient: true }))

    const response = await userBillingHandler(quoteRequest())

    expect(response.status).toBe(200)
    await expect(response.json()).resolves.toMatchObject({ charge: '1200.00', sufficient: true })
  })

  it('reports an outstanding debt as insufficient even when the points balance covers the charge', async () => {
    mocks.issueMeteredScheduleQuote.mockResolvedValue(quote({ available: '1200.00', sufficient: false }))

    const response = await userBillingHandler(quoteRequest())

    expect(response.status).toBe(200)
    await expect(response.json()).resolves.toMatchObject({ charge: '1200.00', available: '1200.00', sufficient: false })
  })
})
