...
export const requestSchemas = {
  ...
  adminCdkCreate: strict({
    ...
    profile_duration: z.enum(['lifetime', 'month', 'half_year', 'year']).optional(),
    amount: optionalString(32),
    item_code: z.enum(['lifetime_profile_voucher', 'limited_profile_voucher']).optional(),
    item_validity_mode: z.enum(['never', 'days', 'date']).optional(),
    item_validity_days: z.number().int().min(1).max(3650).optional(),
    item_expires_at: optionalString(64),
    validity_days: z.number().int().min(1).max(3650).optional(),
    expires_at: optionalString(64),
  ...
  adminServiceStatusCreate: z.discriminatedUnion('action', [
    ...
    strict({
      action: z.literal('save_cost_config'),
      component_id: z.literal('optimization'),
      ...
      valley_worker_instances: z.number().int().min(0).max(100),
      peak_windows: z.array(strict({
        start: z.string().regex(/^(?:(?:[01]\d|2[0-3]):[0-5]\d|24:00)$/),
        end: z.string().regex(/^(?:(?:[01]\d|2[0-3]):[0-5]\d|24:00)$/),
        worker_instances: z.number().int().min(0).max(100),
      })).max(24),
      expected_updated_at: z.string().datetime().nullable(),
      reason: adminOperationReasonSchema,
    }),
  ]),
  adminServiceStatusPatch: strict({
    action: z.literal('append_update'),
    incident_id: shortString(128),
    status: z.enum(['investigating', 'identified', 'monitoring', 'resolved']),
    body: z.string().trim().min(2).max(2000),
    expected_updated_at: z.string().datetime(),
    reason: adminOperationReasonSchema,
...
const ROUTE_POLICIES = new Map<string, RoutePolicy>([
  ...
  ['/api/admin/service-status', route({
    GET: none(),
    POST: json('admin', requestSchemas.adminServiceStatusCreate),
